...
function resolve(source, file, options) {
  console.log("cjs.resolve",{source, file, options})
  var _a, _b, _c, _d;
...
function initMappers(options) {
  console.log("initMappers", options)
  ...
  mappers = configPaths.reduce((paths, path2) => [...paths, ...isGlob__default["default"](path2) ? glob.sync(path2) : [path2]], []).map((v) => {
    console.log("tsconfig v",v)
    return v;
  }).map(tsconfigPaths.loadConfig).filter(isConfigLoaderSuccessResult).map((configLoaderResult) => {
