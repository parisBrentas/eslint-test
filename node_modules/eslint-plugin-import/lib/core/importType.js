...
function baseModule(name) {
    if (isScoped(name)) {
        var _name$split = name.split('/'), _name$split2 = _slicedToArray(_name$split, 2), scope = _name$split2[0],
            _pkg = _name$split2[1];
...
function isInternalRegexMatch(name, settings) {
    ...
    console.log("internalScope", internalScope)
...
function isExternalModule(name, path, context) {
    ...
    console.error(JSON.stringify({name: name, isModule: isModule(name), isScoped: isScoped(name), typeTest: typeTest(name, context,path)}));
...
function isExternalPath(path, context) {
    ...
    console.log("path", (0, _path.relative)(packagePath, path))
...
function isExternalLookingName(name) {
    console.log("isModule", isModule(name), name)
    console.log("isScoped", isScoped(name), name)
    return isModule(name) || isScoped(name);
}

function typeTest(name, context, path) {
    console.log("typeTest", name, context, path)
    var settings = context.settings;
    if (isInternalRegexMatch(name, settings)) {
        console.log("internalRegex")
        return 'internal';
    }
...
function resolveImportType(name, context) {
    console.log("resolverFunction", (0, _resolve2['default']))
