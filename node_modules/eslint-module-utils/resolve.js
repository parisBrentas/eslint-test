...
function tryRequire(target, sourceFile) {
  console.log("tryRequire", {target,sourceFile})
  let resolved;
  try {
    ...
    console.log("(end) tryRequire: not found")
    return undefined;
  }

  // If the target exists then return the loaded module
  console.log("(end) tryRequire: module found, trying to load")
...
function fullResolve(modulePath, sourceFile, settings) {
  console.log("fullResolve", {modulePath, sourceFile, settings})
  // check if this is a bonus core module
  const coreSet = new Set(settings['import/core-modules']);
  if (coreSet.has(modulePath)) {
    console.log("(end) fullResolve: found in core set")
  ...
  if (false) {
    console.log("(end) fullResolve: found in cache", {cachedPath, cacheKey, cacheSettings})
  ...
  function withResolver(resolver, config) {
    console.log("withResolver", {resolver, config})
    function v1() {
      console.log("v1")
      try {
    ...
    function v2() {
      console.log("v2", { resolver, resolve: resolver.resolve, modulePath, sourceFile, config})
  ...
  console.log("configResolvers", configResolvers)
  ...
  for (const pair of resolvers) {
    const name = pair[0];
    const config = pair[1];
    console.log("resolver pair", {name,config,sourceFile})
...
function requireResolver(name, sourceFile) {
  console.log("requireResolver", {name,sourceFile})
  ...
  try {
    ...
    console.log("(end) requireResolver: failed to load resolver", e)
    throw e
  }

  console.log("(end) requireResolver: loaded resolver", resolver)
  return resolver;
}
